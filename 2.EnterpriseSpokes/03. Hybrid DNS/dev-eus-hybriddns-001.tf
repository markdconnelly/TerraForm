...
#region Network
resource "azurerm_virtual_network" "vnet-eus-hybriddns-01" {
  name                = "vnet-eus-hybriddns-01"
...
resource "azurerm_subnet" "subnet-eus-hybriddns-inbound" {
  name                 = "subnet-eus-hybriddns-inbound"
  resource_group_name  = azurerm_resource_group.Ent_CUS_HybridDNS_RG
  virtual_network_name = azurerm_virtual_network.vnet-eus-hybriddns-01.name
  address_prefixes     = ["172.17.1.0/25"]
  delegation {
...
resource "azurerm_subnet" "subnet-eus-hybriddns-outbound" {
  name                 = "subnet-eus-hybriddns-outbound"
  resource_group_name  = azurerm_resource_group.Ent_CUS_HybridDNS_RG
  virtual_network_name = azurerm_virtual_network.vnet-eus-hybriddns-01.name
  address_prefixes     = ["172.17.1.128/25"]
    delegation {
    name = "Microsoft.Network.dnsResolvers"
    service_delegation {
      actions = ["Microsoft.Network/virtualNetworks/subnets/join/action"]
...
resource "azurerm_network_security_group" "nsg-eus-hybriddns-01" {
  name                = "nsg-eus-hybriddns-01"
  location            = azurerm_virtual_wan_hub.vHub-EUS-01.location
  resource_group_name = azurerm_resource_group.ent_cus_hybriddns_rg.name
}
...
#region DNS Resolver
resource "azurerm_private_dns_resolver" "dns-eus-hybriddns-01" {
  name                = "dns-eus-hybriddns-01"
  resource_group_name = azurerm_resource_group.ent_cus_hybriddns_rg.name
  location            = azurerm_virtual_wan_hub.vHub-EUS-01.location
  virtual_network_id  = azurerm_virtual_network.vnet-eus-hybriddns-01.id
}

resource "azurerm_private_dns_resolver_inbound_endpoint" "dns-eus-hybriddns-01-inbound" {
  name                    = "dns-eus-hybriddns-01-inbound"
  private_dns_resolver_id = azurerm_private_dns_resolver.dns-eus-hybriddns-01.id
  location                = azurerm_virtual_wan_hub.vHub-EUS-01.location
  ip_configurations {
    private_ip_allocation_method = "Static"
    private_ip_address          = "172.17.1.6"
    subnet_id                    = azurerm_subnet.subnet-eus-hybriddns-inbound.id
  }
}

resource "azurerm_private_dns_resolver_outbound_endpoint" "dns-eus-hybriddns-01-outbound" {
  name                    = "dns-eus-hybriddns-01-outbound"
  private_dns_resolver_id = azurerm_private_dns_resolver.dns-eus-hybriddns-01.id
  location                = azurerm_virtual_wan_hub.vHub-EUS-01.location
  subnet_id               = azurerm_subnet.subnet-eus-hybriddns-outbound.id
}

resource "azurerm_private_dns_resolver_dns_forwarding_ruleset" "dnsfwd-eus-onprem-ruleset" {
  name                                       = "dnsfwd-eus-onprem-ruleset"
  resource_group_name                        = azurerm_resource_group.ent_cus_hybriddns_rg.name
  location                                   = azurerm_virtual_wan_hub.vHub-EUS-01.location
  private_dns_resolver_outbound_endpoint_ids = [azurerm_private_dns_resolver_outbound_endpoint.dns-eus-hybriddns-01-outbound.id]
}

resource "azurerm_private_dns_resolver_forwarding_rule" "fwdrule-eus-onprem-com" {
  name                      = "fwdrule-eus-onprem-com"
  dns_forwarding_ruleset_id = azurerm_private_dns_resolver_dns_forwarding_ruleset.dnsfwd-eus-onprem-ruleset.id
  domain_name               = "onprem.com."
  enabled                   = true
  target_dns_servers {
    ip_address = ["172.17.0.6", "172.17.0.7", "172.16.0.6"]
    port       = 53
